...
function runAiTurn(game) {
  ...
  while (playedSomething && !game.over) {
    ...
    const options = ai.hand
      ...
      .filter(o => o.card.cost <= ai.manaCur)
      .sort((a, b) => b.card.cost - a.card.cost);
    for (const opt of options) {
      if (opt.card.type === 'minion' && ai.board.length < BOARD_LIMIT) {
        playCard(game, 'ai', opt.idx, null);
        playedSomething = true;
        break;
      }
      if (opt.card.type === 'spell' && !requiresMinionTarget(opt.card)) {
        playCard(game, 'ai', opt.idx, null);
        playedSomething = true;
        break;
      }
      if (opt.card.type === 'spell' && requiresMinionTarget(opt.card) && game.player.board.length > 0) {
        const target = game.player.board.reduce((best, m) => (!best || m.health > best.health) ? m : best, null);
        playCard(game, 'ai', opt.idx, target.uid);
        playedSomething = true;
        break;
      }
    }
